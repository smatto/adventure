...
class Hero:
    ...
    def heal_self(self):
        ...
        time.sleep(1)
        print("You healed yourself for %d HP, but used half your XP. Feels good, man." % int(amount))
        self.xp *= 0.5
        self.hp_limit()
...
class Game:
    ...
    def list_commands(self):
        cmds = ', '.join(self.command_list[:-1])
        print('Commands are %s and %s.' % (cmds, self.command_list[-1]))
    ...
    def update(self):
        ...
        if self.hero.hp <= 0:
            self.hero.death()
            time.sleep(2)
            # restart the game loop by reinitializing global game
            globals()['game'] = Game()
